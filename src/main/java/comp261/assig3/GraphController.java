...
public class GraphController {
    ...
    // Vehicle type sensitivity buttons
    @FXML
    ...
    public void initialize() {
        ...
        // Vehicle type setups
        vehicleTypes = new ToggleGroup();
        bus.setToggleGroup(vehicleTypes);
        car.setToggleGroup(vehicleTypes);
        moped.setToggleGroup(vehicleTypes);

        bus.setSelected(true);
        car.setSelected(false);
        moped.setSelected(false);
    }
    ...
    // Vehicle type sensitivity
    public void handleVehicleType(ActionEvent event) {
        RadioButton selectedRadioButton = (RadioButton) vehicleTypes.getSelectedToggle();
        String toggleGroupValue = selectedRadioButton.getText();
        System.out.println("Vehicle type: " + toggleGroupValue +  " " + event.getEventType());

        if (toggleGroupValue.equals("Bus")) {
            graph.setVehicleTypeMulti(1);
            System.out.println(graph.getVehicleTypeMulti());
        } else if (toggleGroupValue.equals("Car")) {
            graph.setVehicleTypeMulti(2);
            System.out.println(graph.getVehicleTypeMulti());
        } else if (toggleGroupValue.equals("Moped")) {
            graph.setVehicleTypeMulti(4);
            System.out.println(graph.getVehicleTypeMulti());
        } else {
        }
    }
    ...
    public void handleAugmentationPaths(ActionEvent event) {
        ...
        if (aPaths != null) {
            ...
            for (int i = 0; i < aPaths.size(); i++) {
                ...
                for (Node n : aPaths.get(i).getKey()) {
                    tripText.appendText(n.getId() + ", ");
                }
                tripText.appendText(" Flow Value: ");
                tripText.appendText(aPaths.get(i).getValue().toString());
                    tripText.appendText(" || ");
            }
        } else {
    ...
    public void handleMinCut(ActionEvent event) {
        ...
        if (minCutwithSets != null) {
            ...
            Pair<HashSet<Node>, HashSet<Node>> pair = minCutwithSets.getKey();
            HashSet<Node> key = pair.getKey();
            HashSet<Node> value = pair.getValue();

            tripText.appendText("Min-Cut: " + minCutwithSets.getValue() + " || ");

            tripText.appendText("setS: { ");
            for (Node n : key) {
                tripText.appendText(n.getId() + " " + "(" + n.getName() + ")" + ", ");
            }
            tripText.appendText(" } setT: { ");
            for (Node n : value) {
                tripText.appendText(n.getId() + " " + "(" + n.getName() + ")" + ", ");
            }
        } else {
